                        ...
                        <tbody>
                            <tr>
                                ...
                                <td class="border px-4 py-2">
                                    ...
                                    {{-- Pengecekan setiap Role dan Permission dengan if --}}
                                    ...
                                    @if ( auth()->user()->hasRole('admin')||auth()->user()->can('lihat-tulisan'))
                                        <button type="submit"
                                            ...
                                            font-semibold text-xs text-white uppercase tracking-wides">Lihat
                                    </button>
                                    @endif


                                    {{-- Pengecekan setiap Role dengan if --}}

                                    @if ( auth()->user()->hasRole('admin')||auth()->user()->hasRole('penulis'))
                                        <button type="submit"
                                            class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md 
                                            font-semibold text-xs text-white uppercase tracking-wides">Edit
                                        </button>
                                        <button type="submit"
                                            class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md 
                                            font-semibold text-xs text-white uppercase tracking-wides">Delete
                                        </button>
                                    @endif
